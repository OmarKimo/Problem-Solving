...
int solve(int l, int s) {
	if (l == 0 || s < 0) return s == 0;
	int& ret = dp[l][s];
	if (~ret) return ret;
	ret = false;
	lop(i, 10) {
		if (l == len && len != 1 && !i) continue;
		ret |= solve(l - 1, s - i);
	}
	return ret;
}
...
int main()
{
	...
	cin.tie(NULL);
	cin >> len >> sum;
	if (len == 1 && !sum) return puts("0 0");
	mem(dp, -1);
	if(!solve(len, sum)) return puts("-1 -1");
	getMax(len, sum);
	getMin(len, sum);
	cout << mn << ' ' << mx << endl;
