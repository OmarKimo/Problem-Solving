...
int main()
{
    ...
    int ans=0,a,b,p12=0,p13=0,p15=0,p22=0,p23=0,p25=0;
    sc(a), sc(b);
    if(a == b) return puts("0");
    int tmp1 = a, tmp2 = b;
    while(tmp1 && tmp1%2 == 0) p12++,tmp1/=2;
    while(tmp1 && tmp1%3 == 0) p13++,tmp1/=3;
    while(tmp1 && tmp1%5 == 0) p15++,tmp1/=5;
    while(tmp2 && tmp2%2 == 0) p22++,tmp2/=2;
    while(tmp2 && tmp2%3 == 0) p23++,tmp2/=3;
    while(tmp2 && tmp2%5 == 0) p25++,tmp2/=5;
    if(tmp1 != tmp2) return puts("-1");
    cout<<abs(p12-p22)+abs(p13-p23)+abs(p15-p25)<<endl;
