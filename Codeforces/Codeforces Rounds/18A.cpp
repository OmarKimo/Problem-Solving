...
bool isRight(int dis1, int dis2, int dis3) { return dis1 && dis2 && dis3 && (dis1+dis2 == dis3 || dis1+dis3 == dis2 || dis2+dis3==dis1); }
int dis(ii p1, ii p2){ return (p1.f-p2.f)*(p1.f-p2.f) + (p1.s-p2.s)*(p1.s-p2.s); }
...
{	
	...
	vii v(3);
	lop(i,3) cin>>v[i].first>>v[i].second;
	int d1 = dis(v[0],v[1]),d2 = dis(v[1],v[2]),d3 = dis(v[0],v[2]);
	if(isRight(d1,d2,d3)) return puts("RIGHT");
	lop(i,3){
		lop(j,4){
			v[i].first += dx[j], v[i].second += dy[j];
			d1 = dis(v[0],v[1]), d2 = dis(v[1],v[2]), d3 = dis(v[0],v[2]);
			if(isRight(d1,d2,d3)) return puts("ALMOST");
			v[i].first -= dx[j], v[i].second -= dy[j];
		}
	}
	cout<<"NEITHER\n";
