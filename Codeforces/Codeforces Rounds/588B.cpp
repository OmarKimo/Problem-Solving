...
vll vDivisors(ll num)
{
	ll square_root = ll(sqrt(ld(num))) + 1;
	vll arr;
	for (ll i = 1; i < square_root; i++){
		if (!(num%i) && i*i != num) {arr.PB(i); arr.PB(num/i);}
		if (!(num%i) && i*i == num) arr.PB(i);
	}
	sort(all(arr));
	return arr;
}
...
int main()
{
    ...
    ll n;
    scl(n);
    vll divs = vDivisors(n);
    rlop(i,sz(divs)){
        flag = 0;
        primeFactors(divs[i]);
        if(!flag) return cout<<divs[i]<<endl, 0;
    }
