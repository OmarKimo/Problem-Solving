...
int main()
{
	...
	//144B
	map<ii,int> vis;
	int ans, x1,y1,x2,y2,n,x,y,r;
	cin>>x1>>y1>>x2>>y2>>n;
	ans = 2*(abs(y1-y2)+abs(x1-x2)); 
	lop(i,n){
		cin>>x>>y>>r;
		r*=r;
		loop(j,min(y1,y2),max(y1,y2)+1){
			if(!vis[mP(x1,j)] && dis(x1,j,x,y) <= r) ans--, vis[mP(x1,j)]++;
			if(!vis[mP(x2,j)] && dis(x2,j,x,y) <= r) ans--, vis[mP(x2,j)]++;
		}
		loop(j,min(x1,x2)+1, max(x1,x2)){
			if(!vis[mP(j,y1)] && dis(j,y1,x,y) <= r) ans--, vis[mP(j,y1)]++;
			if(!vis[mP(j,y2)] && dis(j,y2,x,y) <= r) ans--, vis[mP(j,y2)]++;
		}
	}
	cout<<ans<<endl;
