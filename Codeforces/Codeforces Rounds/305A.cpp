...
{	
	//
	int cnt1=0,cnt2=0,n,f1=0,f2=0,f3=0;
	cin>>n;
	vi v(n),ans;
	lop(i,n){
		cin>>v[i];
		if(!v[i] || v[i] == 100) ans.PB(v[i]);
		if(v[i] % 10 == 0 && v[i] && v[i] < 100) f1 = v[i];
		else if(v[i]%10 && v[i] < 10) f2 = v[i];
		else if(v[i]%10) f3 = v[i];
	}
	if(f1 && f2) ans.PB(f1), ans.PB(f2);
	else if(f1) ans.PB(f1);
	else if(f3) ans.PB(f3);
	else if(f2) ans.PB(f2);
	cout<<sz(ans)<<endl;
