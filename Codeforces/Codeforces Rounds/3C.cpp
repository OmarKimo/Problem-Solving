...
int main()
{
    ...
    char mat[3][3];
    int c1=0,c2=0;
    lop(i,3) lop(j,3) cin>>mat[i][j], c1+=mat[i][j] == 'X', c2 += mat[i][j] == '0';
    if(abs(c1-c2) > 1 || c2 > c1) return puts("illegal");
    int won1=0,won2=0;
    lop(i,3){
        int f = 1;
        lop(j,3) f &= mat[i][j] == 'X';
    ...
    won1 |= (mat[0][0] == 'X' && mat[1][1] == 'X' && mat[2][2] == 'X');
    won2 |= (mat[0][0] == '0' && mat[1][1] == '0' && mat[2][2] == '0');
    won1 |= (mat[0][2] == 'X' && mat[1][1] == 'X' && mat[2][0] == 'X');
    won2 |= (mat[0][2] == '0' && mat[1][1] == '0' && mat[2][0] == '0');
    if(won1 && won2 || (won1 && c1 == c2) || (won2 && c1 != c2)) return puts("illegal");
    if(won1) return puts("the first player won");
    if(won2) return puts("the second player won");
    if(c1+c2 == 9) return puts("draw");
    if(c1 > c2) return puts("second");
    cout<<"first\n";
