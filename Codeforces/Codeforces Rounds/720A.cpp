...
int main()
{
    ...
    sc(n), sc(m);
    multiset<int> K, L;
    sc(k); lop(i,k) sc(num), K.insert(num);
    sc(l); lop(i,l) sc(num), L.insert(num);
    rlop(i,n){
        lop(j,m){
            int r = i+1, c = j+1;
            auto it = K.lower_bound(r+c);
            if(sz(K) && it != K.end()) K.erase(it);
            else{
                it = L.lower_bound(r+m+1-c);
                if(sz(L) && it != L.end()) L.erase(it);
            }
        }
    }
    if(!sz(K) && !sz(L)) cout<<"YES\n";
    else cout<<"NO\n";
