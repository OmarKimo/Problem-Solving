...
int main()
{
    ...
    queue<int> fi,se;
    lop(i,k) sc(tmp[i]);
    lop(i,k) fi.push(tmp[i]);
    sc(k); lop(i,k) sc(tmp[i]);
    lop(i,k) se.push(tmp[i]);
    while(sz(fi) && sz(se) && cnt < 1000){
        cnt++;
        int n1 = fi.front(), n2 = se.front();
        fi.pop(), se.pop();
        if(n1 > n2) fi.push(n2), fi.push(n1);
        else se.push(n1), se.push(n2);
    }
    if(sz(fi) && sz(se)) cout<<-1<<endl;
    else {
        cout<<cnt<<' ';
        if(sz(fi)) cout<<1<<endl;
        else cout<<2<<endl;
    }
