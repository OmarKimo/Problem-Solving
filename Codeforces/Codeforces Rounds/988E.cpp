...
vi v[10];
lop(i, n) v[num[i] - '0'].PB(i);
int ans = oo;
lop(i, 4){
if(not sz(v[pat[i][0]-'0']) or not sz(v[pat[i][1]-'0'])) continue;
if(i == 3 and sz(v[0]) < 2) continue;
int i1 = v[pat[i][0]-'0'].back(), i2 = v[pat[i][1]-'0'].back();
if(i == 3) i1 = v[0][sz(v[0]) - 2];
if(i2 < i1) i1--;
int term = (n-i2-1)+(n-i1-2);
if(not i1) {
int idx = i1 + 1;
if(i2 == idx) idx++;
while(idx < n and idx != i2 and num[idx++] == '0') term++;
} 
else if(not i2) {
int idx = i2 + 1;
if(i1 == idx) idx++;
while(idx < n and idx != i1 and num[idx++] == '0') term++;
} 
ans = min(ans, term);
}
