...
int main()
{
	...
	cin>>str;
	vi idxs;
	lop(i,sz(str)) if(str[i] == '?') idxs.PB(i);
	if(sz(str) < 26) return puts("-1");
	lop(i,sz(str)-25){
		fill(all(vis),false);
		flag = 1;
		lop(j,26) {
			if(str[i+j] != '?' && vis[str[i+j]-'A']) { flag=0; break; }
			else if(str[i+j] != '?') vis[str[i+j]-'A'] = true;
		}
		if(flag){
			int fl=0;
			lop(j,26) if(!vis[j]) fl=1, out += ('A' + j);
			if(!fl) out+='A';
			int k=0;
			lop(j,sz(idxs)) str[idxs[j]] = out[k%sz(out)], k++;
			break;
		}
	}
