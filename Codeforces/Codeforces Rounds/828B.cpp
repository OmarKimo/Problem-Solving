...
int main()
{
	...
	int t = 1;
	while(t--){
        int cnt=0,minX = 1000,minY = 1000,maxX = -1,maxY = -1,n,m;
        char ch;
        cin>>n>>m;
        lop(i,n){
            lop(j,m){
                cin>>ch;
                if(ch == 'B') cnt++,minX = min(minX, i), minY = min(minY, j), maxX = max(maxX,i), maxY = max(maxY,j);
            }
        }
        if(!cnt){
            cout<<"1\n";
        ...
        int l = maxX - minX +1, w = maxY - minY + 1;
        //printf("%d\n%d\n%d\n%d\n%d\n%d\n", minX,maxX,minY,maxY,l,w);
        if(l == w) cout<<l*w - cnt<<endl;
        else{
            if(l < w){
                if(w > n) cout<<-1<<endl;
                else cout<<w*w-cnt<<endl;
            }
            else{
                if(l > m) cout<<-1<<endl;
                else cout<<l*l-cnt<<endl;
            }
        }
	}
