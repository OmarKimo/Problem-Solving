...
{	
	...
	string s;
	cin>>s;
	int ans = 1000,cnt[26] = {0}, req[26] = {0};
	lop(i,sz(s)) cnt[s[i]-'a']++;
	string ss = "", num = "nineteen";
	lop(i,sz(num)) if(++req[num[i]-'a'] == 1) ss+=num[i];
	lop(i,sz(ss)) {
		if(ss[i] == 'n'){
			if(cnt[ss[i]-'a'] < 3) ans = 0;
			else ans = min(ans, 1 + (cnt[ss[i]-'a']-3)/2);
			continue;
		}
		ans = min(ans, cnt[ss[i]-'a']/req[ss[i]-'a']);
	}
	cout<<ans<<endl;
