...
int main()
{
    ...
    int des, x1,x2,t1,t2,p,d;
    sc(des),sc(x1), sc(x2),sc(t1),sc(t2),sc(p),sc(d);
    if(t1 >= t2) return cout<<abs(x1-x2)*t2<<endl, 0;
    int val1 = abs(x1-x2)*t2;
    int val = abs(x1-p);
    if(x1 > p && d == -1) val += 2*p, d = 1;
    else if(x1 < p && d == 1) val += 2*(des-p), d = -1;
    val *= t1;
    p = x1, x1 = x2;
    int val2 = abs(x1-p);
    if(x1 > p && d == -1) val2 += 2*p;
    else if(x1 < p && d == 1) val2 += 2*(des-p);
    val2 *= t1;

    cout<<min(val1, val+val2)<<endl;
