...
int main()
{
    ...
    if(v[mid] == x) cout<<"0\n";
        ...
        cnt = 0;
        int pos,idx = -1;
        rlop(i,mid+1) if(v[i] <= x) { idx = i; break; }
        if(idx == -1) pos = 1, n++,cnt++;
        else {
            pos = idx + 1;
            if(v[idx] != x) pos++, n++, cnt++;
        }
        while((n+1)/2 != pos) pos++,n++,cnt++;
        cout<<cnt<<endl;
    }
    else{
        cnt = 0;
        int idx = -1, pos;
        loop(i,mid,n) if(v[i] >= x) { idx = i; break; }
        if(idx == -1) pos = n+1, n++,cnt++;
        else {
            pos = idx+1;
            if(v[idx] != x) n++, cnt++;
        }
        while((n+1)/2 != pos) n++,cnt++;
        cout<<cnt<<endl;
    }
