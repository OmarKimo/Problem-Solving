...
set<ll> st;
ll x;
lop(i, n) cin>>x, st.insert(x);
bool found = false;
for(auto num : st) {
if(found) break;
for(int p = 0; p < 31; p++){
ll num1 = num - (1ll<<p), num2 = num + (1ll<<p);
if(st.count(num1)and st.count(num2)){
found = true;
cout<<3<<endl<<num<<' ' <<num1<<' ' <<num2<<endl;
break;
}
} 
} 
...
if(st.count(num1)){
found = true;
cout<<2<<endl<<num<<' ' <<num1<<endl;
break;
} 
} 
} 
if(not found) cout<<1<<endl<<*st.begin()<<endl;
} 
} 
