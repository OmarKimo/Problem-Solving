...
int k;
cin>>k;
vector<pair<pair<ll,int>, int>> table;
lop(i, k) {
int n;
ll sum = 0;
cin>>n;
vi v(n);
lop(j, n) cin>>v[j], sum += v[j];
lop(j, n) table.PB({{sum-v[j], i+1}, j+1});
} 
sort(all(table));
bool f = false;
lop(i, sz(table) - 1)
if(table[i].f.f == table[i+1].f.f and table[i].f.s != table[i+1].f.s){
f = true;
cout<<"YES\n";
cout<<table[i].f.s<<' ' <<table[i].s<<endl;
cout<<table[i+1].f.s<<' ' <<table[i+1].s<<endl;
break;
}
if(not f) cout<<"NO\n";
} 
