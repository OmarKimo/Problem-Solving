...
int BFSPath(int src) {
	...
	vi par(100010, -1);
	queue<int> q;
	q.push(src), len[src] = 0;

	int dep = 0, cur = src, sz = 1;
	bool ok = true;

	for ( ; ok &&  !q.empty();	++dep, sz = q.size()) {
		while (ok && sz--) {
			cur = q.front(), q.pop();
			lop(i, 2) { int val = (i)?(2*cur):(cur-1);
			if (len[val] == oo)
			{
				q.push(val), len[val] = dep+1, par[val] = cur;

				if(val == m)	// we found target no need to continue
				{
					ok = false;
					break;
				}
