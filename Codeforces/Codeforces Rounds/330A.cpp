...
int main()
{
	...
	int r,c,ans=0;
	cin>>r>>c;
	vvc mat(r,vc(c)); vvi vis(r,vi(c,0));
	lop(i,r) lop(j,c) cin>>mat[i][j];

	lop(i,r){ 
		int cnt = 0;
		lop(j,c){
			if(mat[i][j] == 'S') { fill(all(vis[i]),0); cnt=0; break; }
			if(!vis[i][j]) cnt++,vis[i][j]=1;
		}
		ans+=cnt;
	}
	...
	lop(i,c) { 
		int cnt = 0; 
		lop(j,r){
			if(mat[j][i] == 'S') { lop(k,j) vis[j][i]=0; cnt = 0; break; }
			if(!vis[j][i]) cnt++, vis[j][i]=1;
		}
		ans+=cnt;
	}
	cout<<ans<<endl;
