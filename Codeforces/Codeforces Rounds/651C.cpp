...
int main()
{
    ...
    sc(n);
    vii v(n);
    mii X,Y; map<ii,int> XY;
    set<int> xs,ys; set<ii> xy;
    lop(i,n) sc(x), sc(y), X[x]++, Y[y]++, XY[ii(x,y)]++, xs.insert(x), ys.insert(y),xy.insert(ii(x,y));
    ll ans = 0;
    for(auto it : xs) if(X[it] > 1) ans += nCr(X[it],2);
    for(auto it : ys) if(Y[it] > 1) ans += nCr(Y[it],2);
    for(auto it : xy) if(XY[it] > 1)ans -= nCr(XY[it],2);
    cout<<ans<<endl;
