...
const int N = 100010;
ll dp[N][2]; int mx,cnt[N];
ll solve(int num, int flag){
    if(num == mx) return 0;
    if(dp[num][flag] != -1) return dp[num][flag];
    dp[num][flag] = 0;
    if(flag) dp[num][flag] = 1LL*num*cnt[num] + solve(num+1,false);
    return dp[num][flag] = max(dp[num][flag], solve(num+1,true));
}
...
int main()
{
    ...
    while(t--){
        sc(n);
        mem(dp,-1), mem(cnt,0);
        lop(i,n) sc(num), mx = max(mx,num), cnt[num]++;
        mx++;
        cout<<solve(1,1)<<endl;
    }
