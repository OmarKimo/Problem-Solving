...
{	
	...
	lop(i,12) mp[arr[i]] = i+1;
	lop(i,3){
		string X,Y1,Z1,Y2,Z2;
		X = v[i];
		Y1 = v[(i+1)%3], Y2 = v[(i+2)%3];
		Z1 = v[(i+2)%3], Z2 = v[(i+1)%3];
		int dis1,dis2;
		if(mp[X] <= mp[Y1]) dis1 = mp[Y1]-mp[X];
		else dis1 = 12-mp[X] + mp[Y1];
		if(mp[Y1] <= mp[Z1]) dis2 = mp[Z1]-mp[Y1];
		else dis2 = 12-mp[Y1] + mp[Z1];
		if(dis1 == 3 && dis2 == 4) return puts("minor");
		if(dis1 == 4 && dis2 == 3) return puts("major");

		if(mp[X] <= mp[Y2]) dis1 = mp[Y2]-mp[X];
		else dis1 = 12-mp[X] + mp[Y2];
		if(mp[Y2] <= mp[Z2]) dis2 = mp[Z2]-mp[Y2];
		else dis2 = 12-mp[Y2] + mp[Z2];
		if(dis1 == 3 && dis2 == 4) return puts("minor");
		if(dis1 == 4 && dis2 == 3) return puts("major");
	}
	cout<<"strange\n";
