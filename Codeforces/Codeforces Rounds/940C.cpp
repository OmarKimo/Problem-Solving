...
{	
	...
	cin>>s;
	set<char> st;
	lop(i,n) st.insert(s[i]);
	set<char>::iterator it = st.end(); it--;
	if(k > n){
		t=s;
		for(int i=0; i<k-n; i++) t+=*(st.begin());
		cout<<t<<endl;
		return 0;
	}
	for(int i=0; i<k; i++) t+=s[i];
	for(char c = 'a'; c<='z'; c++) if(st.count(c) && c > s[k-1]) { t[k-1] = c; break; }
	int flag=1;
	if(t[k-1] == s[k-1]){
		flag=0;
		t[k-1] = *(st.begin());
		for(int i = k-2; i>=0; i--){
			...
			if(!flag) t[i] = *(st.begin());
			else break;
		}
	}
	if(!flag){
		for(char c = s[0]+1; c<='z'; c++) if(st.count(c)) { t[0] = c; break; }
	}
	cout<<t<<endl;
