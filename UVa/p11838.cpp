...
vvi graph(2010); vi vis;
void dfs(int node){
	vis[node-1]=1;
	lop(i,sz(graph[node])) if(!vis[graph[node][i]-1]) dfs(graph[node][i]);
}
bool my(int j) { return j==1; }
...
int main()
{
	...
	while(cin>>n>>m, n,m){
		flag=1;
		clr(vis), rsz(vis)(n,0), clr(graph), rsz(graph)(2010);
		lop(i,m) { sc(u), sc(v), sc(p), graph[u].PB(v); if(p==2) graph[v].PB(u); }
		loop(i,1,n+1) {
			fill(all(vis),0);
			dfs(i);
			if (!all_of(all(vis),my)) flag=0;
		}
		cout<<flag<<endl;
	}
